...
pub fn sys_close(fd: usize) -> Result<(), i32> {
    let ret = unsafe {
        raw_syscall(3 /*SYS_CLOSE*/, fd, 0, 0, 0, 0, 0)
    };
    check_ret(ret).map(|_| ())
}
...
pub fn for_each_dirent64<F>(buf: &[u8], len: usize, mut visit: F)
where
    F: FnMut(&str, u8),
{
    ...
    while offset < limit {
        ...
        if remaining < DIRENT64_HEADER_LEN {
            break;
        }

        let reclen_at = offset + DIRENT64_RECLEN_OFFSET;
        let rec_len = u16::from_le_bytes([buf[reclen_at], buf[reclen_at + 1]]) as usize;
        if rec_len < DIRENT64_HEADER_LEN || rec_len > remaining {
            break;
        }

        let d_type = buf[offset + DIRENT64_TYPE_OFFSET];
        ...
        if name_len > 0 {
            if let Ok(name) = core::str::from_utf8(&name_region[..name_len]) {
                visit(name, d_type);
            }
        }
...
pub fn sys_getpid() -> usize {
    let ret = unsafe {
        raw_syscall(39 /*SYS_GETPID*/, 0, 0, 0, 0, 0, 0)
    };
    ret as usize
}
...
pub fn sys_getppid() -> usize {
    let ret = unsafe {
        raw_syscall(110 /*SYS_GETPPID*/, 0, 0, 0, 0, 0, 0)
    };
    ret as usize
}
...
pub fn sys_fork() -> Result<usize, i32> {
    let ret = unsafe {
        raw_syscall(57 /*SYS_FORK*/, 0, 0, 0, 0, 0, 0)
    };
    check_ret(ret)
}
...
pub fn sys_exit(code: i32) -> ! {
    unsafe {
        raw_syscall(60 /*SYS_EXIT*/, code as usize, 0, 0, 0, 0, 0)
    };
    loop {}
}
...
pub fn sys_dup2(old_fd: usize, new_fd: usize) -> Result<usize, i32> {
    let ret = unsafe {
        raw_syscall(33 /*SYS_DUP2*/, old_fd, new_fd, 0, 0, 0, 0)
    };
    check_ret(ret)
}
...
pub fn sys_pipe(fds: &mut [usize; 2]) -> Result<(), i32> {
    let ret = unsafe {
        raw_syscall(22 /*SYS_PIPE*/, fds.as_ptr() as usize, 0, 0, 0, 0, 0)
    };
    check_ret(ret).map(|_| ())
}
...
pub fn sys_getuid() -> u32 {
    let ret = unsafe {
        raw_syscall(102 /*SYS_GETUID*/, 0, 0, 0, 0, 0, 0)
    };
    ret as u32
}
...
pub fn sys_getgid() -> u32 {
    let ret = unsafe {
        raw_syscall(104 /*SYS_GETGID*/, 0, 0, 0, 0, 0, 0)
    };
    ret as u32
}
...
pub fn sys_time() -> i64 {
    let ret = unsafe {
        raw_syscall(201 /*SYS_TIME*/, 0, 0, 0, 0, 0, 0)
    };
    ret as i64
}
...
pub fn sys_kill(pid: usize, sig: i32) -> Result<(), i32> {
    let ret = unsafe {
        raw_syscall(62 /*SYS_KILL*/, pid, sig as usize, 0, 0, 0, 0)
    };
    check_ret(ret).map(|_| ())
}
