...
DISK = os.path.join(base_dir, 'images', 'echos-efi.img')
EFI = os.path.join(base_dir, '..', '..', '..', '..', 'target', 'x86_64-unknown-uefi', 'debug', 'ech_os.efi')
...
with open(DISK, 'wb') as f:
    ...
    for i in range(efi_clusters - 1):
        fat[12 + i*4 : 16 + i*4] = struct.pack('<I', efi_start + i + 1)
    fat[12 + (efi_clusters-1)*4 : 16 + (efi_clusters-1)*4] = b'\xFF\xFF\xFF\x0F'
