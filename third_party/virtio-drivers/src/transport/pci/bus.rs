...
fn pio_address(device_function: DeviceFunction, register_offset: u8) -> u32 {
    let bdf = (device_function.bus as u32) << 16
        | (device_function.device as u32) << 11
        | (device_function.function as u32) << 8;
    0x8000_0000 | bdf | (register_offset as u32 & 0xfc)
}
...
unsafe fn pio_read_config(device_function: DeviceFunction, register_offset: u8) -> u32 {
    outl(PCI_CONFIG_ADDRESS, pio_address(device_function, register_offset));
    inl(PCI_CONFIG_DATA)
}

unsafe fn pio_write_config(device_function: DeviceFunction, register_offset: u8, data: u32) {
    outl(PCI_CONFIG_ADDRESS, pio_address(device_function, register_offset));
...
impl PciRoot {
    ...
    pub(crate) fn config_read_word(
        ...
        match self.cam {
            Cam::Pio => unsafe { pio_read_config(device_function, register_offset) },
            _ => {
    ...
    pub(crate) fn config_write_word(
        ...
        match self.cam {
            Cam::Pio => unsafe { pio_write_config(device_function, register_offset, data) },
            _ => {
                let address = self.cam_offset(device_function, register_offset);
                unsafe {
