...
void virtio_disk_init(uint16_t base_port) {
    ...
    /* Adım 5: Özellik müzakeresi — bu basit sürücü hiçbir ek özellik istemez */
 ...
 *  Gerçek bir sürücüde kesinti (interrupt) kullanılır; bu basit uygulama
 *  polling yaparak CPU'yu meşgul tutar ama sürün sayısı sınırlıdır (~50M).
 *
...
void virtio_disk_rw(uint64_t sector, void* buf, int write) {
    ...
    desc[1].len   = 512; /* Standart blok boyutu: 512 bayt */
