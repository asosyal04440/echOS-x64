...
impl SystemTopology {
    ...
    fn detect_basic_info(&mut self) -> Result<(), TopologyError> {
        ...
        // ECX ve EDX feature bitleri — birleştirilmiş 64-bit maske
        let cpu_features = ((cpuid1.ecx as u64) << 32) | (cpuid1.edx as u64);

        // CPUID leaf 0x16: İşlemci frekansı (Hz cinsinden)
        // EAX = base freq MHz, EBX = max freq MHz, ECX = bus freq MHz
        // Bu yaprak tüm işlemcilerde mevcut olmayabilir
        let (base_freq, max_freq) = {
            let max_leaf = unsafe { core::arch::x86_64::__cpuid(0) }.eax;
            if max_leaf >= 0x16 {
                let cpuid16 = unsafe { core::arch::x86_64::__cpuid_count(0x16, 0) };
                let b = if cpuid16.eax > 0 { cpuid16.eax } else { 2400 };
                let m = if cpuid16.ebx > 0 { cpuid16.ebx } else { b };
                (b, m)
            } else {
                // Leaf 0x16 desteklenmiyorsa varsayılan değerler
                (2400u32, 3600u32)
            }
        };
    ...
    fn detect_cache_hierarchy(&mut self) -> Result<(), TopologyError> {
        ...
        for subleaf in 0..16u32 {
            let cpuid4 = unsafe { core::arch::x86_64::__cpuid_count(4, subleaf) };
            let cache_type_bits = cpuid4.eax & 0x1F;
            if cache_type_bits == 0 {
                break; // Daha fazla cache yok
            }
            ...
            let level = ((cpuid4.eax >> 5) & 0x7) as u8;
            let line_size = ((cpuid4.ebx & 0xFFF) + 1) as u16;
            let partitions = (((cpuid4.ebx >> 12) & 0x3FF) + 1) as u32;
            let ways = (((cpuid4.ebx >> 22) & 0x3FF) + 1) as u16;
            let sets = cpuid4.ecx + 1;
            let size = (ways as u32) * partitions * (line_size as u32) * sets;
            let inclusive = (cpuid4.edx & 0x2) != 0;

            let mut desc = CacheDescriptor::new(cache_type, level, size, line_size, ways);
            desc.sets = sets;
            desc.inclusive = inclusive;
            caches.push(desc);
        }

        // Eğer CPUID leaf 4 hiç cache dönmediyse, varsayılan değerler kullan
        if caches.is_empty() {
            caches.push(CacheDescriptor::new(CacheType::Data, 1, 32 * 1024, 64, 8));
            caches.push(CacheDescriptor::new(
        ...
        for cpu_id in 0..self.max_cpus {
            ...
            let mutable_topology = topology_guard.as_mut();
            for cache in &caches {
                mutable_topology.add_cache(*cache);
            }
        }
    ...
    fn detect_smt_info(&mut self) -> Result<(), TopologyError> {
        ...
        let (threads_per_core, cores_per_package) = if max_leaf >= 0xB {
            let smt = unsafe { core::arch::x86_64::__cpuid_count(0xB, 0) };
            let core = unsafe { core::arch::x86_64::__cpuid_count(0xB, 1) };

            let tpc = if smt.ebx & 0xFFFF > 0 {
                (smt.ebx & 0xFFFF)
            } else {
                1
            };
            let total_logical = if core.ebx & 0xFFFF > 0 {
                (core.ebx & 0xFFFF)
            } else {
                tpc
            };
            let cpp = total_logical / tpc;
            (tpc.max(1), cpp.max(1))
        } else {
            // Leaf 0xB desteklenmiyorsa leaf 1 ile tahmin et
            let cpuid1 = unsafe { core::arch::x86_64::__cpuid_count(1, 0) };
            let max_logical = ((cpuid1.ebx >> 16) & 0xFF).max(1);
            let htt = (cpuid1.edx >> 28) & 1;
            if htt != 0 && max_logical > 1 {
                // Basit tahmin: HT varsa 2 thread/core
                (2u32, (max_logical / 2).max(1))
            } else {
                (1u32, max_logical)
            }
        };
        ...
        self.threads_per_core
            .store(threads_per_core, Ordering::Release);
        self.cores_per_package
            .store(cores_per_package, Ordering::Release);
    ...
    fn detect_package_info(&mut self) -> Result<(), TopologyError> {
        ...
        self.package_count
            .store(packages.len() as u32, Ordering::Release);
...
pub fn init(max_cpus: u32) -> Result<(), TopologyError> {
    ...
    crate::serial_println!(
        "Topology: Initializing topology detection for {} CPUs",
        max_cpus
    );
...
mod tests {
    ...
    fn test_cpu_topology() {
        ...
        topology.add_cache(CacheDescriptor::new(
            CacheType::Unified,
            3,
            25 * 1024 * 1024,
            64,
