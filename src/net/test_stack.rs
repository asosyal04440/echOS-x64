...
use super::socket::{accept, listen, recvfrom, sendto};
use super::socket::{
    bind, close, connect, recv, send, socket, AddressFamily, Protocol, SocketType,
};
use super::*;
...
fn test_arp_protocol() -> Vec<TestResult> {
    ...
    let arp_request = super::arp::ArpPacket::new_request(src_mac, src_ip, target_ip);
    ...
    match serialized {
        Ok(()) => {
            results.push(TestResult::success(
                ...
                &format!(
                    "Successfully serialized ARP request: {} bytes",
                    super::arp::ArpHeader::SIZE
                ),
            ));
...
fn test_dhcp_client() -> Vec<TestResult> {
    ...
    results.push(TestResult::success(
        ...
        &format!(
            "Successfully serialized DHCP Discover: {} bytes",
            serialized.len()
        ),
    ));
    ...
    if lease.is_none() {
        ...
        results.push(TestResult::success("DHCP Lease Check", "DHCP lease exists"));
    }
    ...
    if let Ok(()) = header.serialize(&mut buf) {
        ...
        if let Ok(q_len) = question.serialize(&mut buf[q_offset..]) {
            results.push(TestResult::success(
                "DNS Query Creation",
                &format!(
                    "Successfully created DNS query: {} bytes total",
                    super::dns::DnsHeader::SIZE + q_len
                ),
            ));
        } else {
            results.push(TestResult::failure(
