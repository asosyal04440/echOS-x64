...
impl HuffmanCodeSymbol {
    fn new(symbol: usize) -> Self {
        if symbol == 256 {
            ...
            Self::Symbol(symbol as u8)
        }
    }
}

struct HuffmanDecoder {
    table: BTreeMap<u8, BTreeMap<u32, HuffmanCodeSymbol>>,
    eos_codepoint: (u32, u8),
}

impl HuffmanDecoder {
    fn from_table(table: &[(u32, u8)]) -> Self {
        let mut decoder_table = BTreeMap::new();
        let mut eos_codepoint = None;

        for (symbol, &(code, code_len)) in table.iter().enumerate() {
            decoder_table
                .entry(code_len)
                .or_insert_with(BTreeMap::new)
                .insert(code, HuffmanCodeSymbol::new(symbol));
            if symbol == 256 {
        ...
        Self {
            table: decoder_table,
            eos_codepoint: eos_codepoint.expect("HPACK EOS codepoint"),
        }
    }

    fn new() -> Self {
        Self::from_table(HPACK_HUFFMAN_CODE_TABLE)
    }
    ...
    fn decode(&mut self, buf: &[u8]) -> Result<Vec<u8>, HuffmanDecodeError> {
        ...
        for bit in BitIterator::new(buf.iter()) {
            ...
            if let Some(length_table) = self.table.get(&current_len) {
                if let Some(symbol) = length_table.get(&current) {
                    match symbol {
                        HuffmanCodeSymbol::Symbol(symbol) => result.push(*symbol),
                        HuffmanCodeSymbol::EndOfString => {
                            return Err(HuffmanDecodeError::EosInString);
                        }
                    }
                    current = 0;
                    current_len = 0;
        ...
        if current_len > 7 {
            return Err(HuffmanDecodeError::PaddingTooLarge);
        }
        ...
        let mask = if current_len == 0 {
            ...
            ((1u32 << current_len) - 1) << (32 - current_len)
        };
...
impl<'a, I: Iterator<Item = &'a u8>> Iterator for BitIterator<'a, I> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let is_set = (byte & (1 << self.pos)) == (1 << self.pos);
        if self.pos == 0 {
...
static HPACK_HUFFMAN_CODE_TABLE: &[(u32, u8)] = &[
    (0x1ff8, 13),
    (0x7fffd8, 23),
    (0xfffffe2, 28),
    (0xfffffe3, 28),
