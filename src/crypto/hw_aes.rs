...
impl ClMulGhash {
    ...
    fn gf_mul(&self, x: [u64; 2]) -> [u64; 2] {
        // İndirgenemez polinom sabiti (bit-tersine çevrilmiş): 0x87
        const REDUCE: u64 = 0x87;

        let mut z0 = 0u64;
        let mut z1 = 0u64;

        // Bit-bit çarpım: x'in her biti için koşullu H ekleme (XOR)
        for i in 0..64 {
            if (x[1] >> (63 - i)) & 1 != 0 {
                z0 ^= self.h[0];
                z1 ^= self.h[1];
            ...
            // h'yi 1 bit sola kaydır (GF(2)'de x ile çarpım)
            let carry = self.h[0] >> 63;
            // Not: self.h değiştirilemez, simüle ediyoruz
        }

        // Basitleştirilmiş Karatsuba stili çarpım
        // Verimlilik için 32-bit yarılara bölünmüş
        let x0 = x[0] >> 32;
        ...
        // Her 32-bitlik parçayı çarp ve birleştir (GF(2)'de toplama = XOR)
        ...
        for bit in 0..128 {
            ...
            if (x[word_idx] >> (63 - bit_pos)) & 1 != 0 {
                // Kaydırılmış H'i sonuca ekle (XOR)
                // Basitleştirilmiş sürüm — tam uygulama dikkatli indirgeme gerektirir
                result[0] ^= self.h[0];
                result[1] ^= self.h[1];
            }
        }

        // İndirgenemez polinom ile modüler indirgeme (GF(2^128) için kritik adım)
        for _ in 0..2 {
            let carry = result[0] >> 63;
            result[0] = result[0].wrapping_shl(1) | (result[1] >> 63);
            result[1] = result[1].wrapping_shl(1);
            if carry != 0 {
                result[1] ^= REDUCE; // Taşma durumunda azaltma polinomuyla XOR
            }
        }
...
/// RDRAND talimatı ile donanım rastgele baytlar üretir.
///
/// RDRAND, Intel Bull Mountain entropi kaynağından beslenen bir CSPRNG'dir.
...
/// RDSEED talimatı ile doğrudan donanım entropisi üretir.
///
/// RDSEED, deterministic bir RBG'den değil doğrudan donanım entropi kaynağından okur.
