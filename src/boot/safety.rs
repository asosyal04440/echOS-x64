...
//!
//! Boot zamanı çökmelerine, TLSF bozulmalarına, SMP hatalarına,
//! IDT sorunlarına ve GOP problemlerine karşı kapsamlı koruma sağlar.
...
/// Boot aşaması zaman aşımı (milisaniye)
pub const BOOT_PHASE_TIMEOUT_MS: u64 = 30000;
/// AP başlatma zaman aşımı (milisaniye)
pub const AP_STARTUP_TIMEOUT_MS: u64 = 5000;
/// Heap bütünlük kontrol aralığı
pub const HEAP_CHECK_INTERVAL: u64 = 1000;
/// Durdurmadan önce izin verilen maksimum heap bozulması sayısı
...
impl BootSafetyState {
    ...
    /// Mevcut aşamadaki hatayı kaydet
    ...
    /// Boot'un çok uzun sürdüğünü kontrol et
...
impl HeapSafety {
    ...
    pub fn check_integrity() -> HeapIntegrityStatus {
        ...
        // Belirgin bozulma işaretlerini kontrol et
        if start > end {
        ...
        // Erken heap taşmasını kontrol et
    ...
    /// Yeniden deneme ile güvenli bellek tahsisi
    pub fn safe_alloc(size: usize, align: usize) -> Option<*mut u8> {
        ...
        for attempt in 0..MAX_RETRY_ATTEMPTS {
            ...
            if !ptr.is_null() {
                // İşaretçinin geçerli heap aralığında olduğunu doğrula
                if Self::is_valid_heap_ptr(ptr as usize) {
                    return Some(ptr);
                } else {
                    // Geçersiz işaretçi - heap bozulması!
                    ...
                    // Bozulma eşiğini aşıp aşmadığını kontrol et
    ...
    fn is_valid_heap_ptr(ptr: usize) -> bool {
        // Erken heap'i kontrol et
        let early_start = 0x1000; // Yaklaşık
        let early_end = early_start + 512 * 1024;
        if ptr >= early_start && ptr < early_end {
            return true;
        }

        // Ana heap'i kontrol et
        let (start, end) = crate::allocator::tlsf::main_heap_bounds();
    ...
    /// Kritik heap hatasında acil durdurma
...
impl SmpSafety {
    /// Kapsamlı hata yönetimi ile güvenli AP başlatma
    pub fn safe_startup_ap(apic_id: u32, cpu_id: u32) -> bool {
        ...
        // Yeniden denemelerle başlatma girişimi
        for attempt in 0..MAX_RETRY_ATTEMPTS {
            crate::serial_println!(
            ...
            // Gerçek başlatmayı çağır
            let result = unsafe { crate::cpu::smp::startup_ap(apic_id, cpu_id) };

            if result {
                // AP'nin gerçekten online olduğunu doğrula
        ...
        // Sistem azaltılmış CPU sayısıyla devam edebilir
        crate::serial_println!(
    ...
    /// AP başlatmadan önce ön kontroller
    fn preflight_checks(cpu_id: u32) -> bool {
        // CPU durumunun başlatmaya izin verip vermediğini kontrol et
        ...
        // CPU başına verinin hazır olup olmadığını kontrol et
        ...
        // Stack'in tahsis edilip edilmediğini kontrol et
    ...
    fn verify_ap_online(cpu_id: u32) -> bool {
        // AP'nin online olduğunu bildirmesini bekle
    ...
    /// AP başlatma hatasını zarif biçimde işle
    pub fn handle_bringup_failure(cpu_id: u32, reason: &str) {
        ...
        // Beklenen online sayısını azalt
...
impl IdtSafety {
    /// Doğrulama ile güvenli IDT başlatma
    pub fn safe_init_idt(cpu_id: u32) -> bool {
        for attempt in 0..MAX_RETRY_ATTEMPTS {
            ...
            // IDT'nin geçerli olduğunu doğrula
    ...
    /// IDT yapısının geçerli olduğunu doğrula
    fn verify_idt(idt: &x86_64::structures::idt::InterruptDescriptorTable) -> bool {
        // Kritik handler'ların ayarlandığını kontrol et
        // Double fault handler mevcut olmalı
        // Page fault handler mevcut olmalı
        // General protection fault handler mevcut olmalı

        // IDT yapısını okuyabiliyorsak geçerlidir
        let ptr = idt as *const _ as usize;
        ptr != 0 && ptr % 8 == 0 // Hizalı olmalı
    }

    /// IDT'nin CPU'ya yüklendiğini doğrula
    fn verify_idt_loaded() -> bool {
        // IDTR'yi oku ve geçerli belleğe işaret ettiğini doğrula
        ...
        // Limit (ilk 2 byte) ve base (sonraki 8 byte) çıkar
        ...
        // IDT en az 32 giriş içermeli
        limit >= 32 * 16 - 1 && base != 0
    }

    /// Güvenli istisna handler'larını kur
    pub fn install_safe_handlers() {
        // Bu fonksiyon tüm istisna handler'larının uygun hata yönetimine
        // sahip olmasını ve double fault'a yol açmamasını sağlar
...
impl GopSafety {
    /// Geri dönüşlerle güvenli GOP başlatma
    pub fn safe_init(framebuffer: Option<&mut crate::boot::Framebuffer>) -> bool {
        // Framebuffer yoksa alternatifler dene
    ...
    /// Framebuffer'ın kullanılabilir olduğunu doğrula
    ...
    /// Metin moduna geri düş
    fn try_text_mode() -> bool {
        crate::serial_println!("[GOP_SAFETY] Falling back to serial output only");
        true // Seri çıkış her zaman çalışır
    }

    /// Sınır kontrolü ile güvenli piksel yazma
...
// ============================================================================
// BOOT İZLEYİCİSİ (WATCHDOG)
// ============================================================================

pub struct BootWatchdog;

impl BootWatchdog {
    /// Boot izleyicisini başlat
    pub fn start() {
    ...
    pub fn check() {
        ...
        // Aşama zaman aşımını kontrol et
        ...
        if current_time.saturating_sub(last_checkpoint as usize) > phase_timeout {
            ...
            // Aşamaya göre kurtarma dene
            Self::attempt_recovery(current_phase);
        }
    }

    /// Zaman aşımından kurtarma dene
    ...
    /// Boot'u tamamlandı olarak işaretle
    pub fn complete() {
